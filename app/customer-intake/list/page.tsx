...
export default function CustomerIntakeListPage() {
    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .select(
            "id,customer_id,name,phone,allergy,ng_items,agreed,signature_data_url,created_at"
          )
      ...
      intakeRows.forEach((intake) => {
        nextSelections[intake.id] =
          intake.customer_id ||
          suggestCustomerId(intake, customerRows) ||
          "";
      });
  ...
  const stats = useMemo(() => {
    ...
    const linked = intakes.filter((item) => Boolean(item.customer_id)).length;
  ...
  async function handleUnlink(intakeId: string) {
    ...
    try {
      ...
      const currentIntake =
        intakes.find((item) => item.id === intakeId) || null;
      ...
      setLinkSelections((previous) => ({
        ...
        [intakeId]: currentIntake
          ? suggestCustomerId(currentIntake, customers) || ""
          : "",
      }));
        ...
        <div className="space-y-4">
          {filteredIntakes.map((intake) => {
            const linkedCustomer =
              customers.find(
                (customer) => customer.id === intake.customer_id
              ) || null;

            const suggestedCustomerId = suggestCustomerId(
              intake,
              customers
            );

            const currentSelection =
              linkSelections[intake.id] || "";
                    ...
                    <div className="rounded-xl border bg-white p-3">
                      ...
                      >
                        ...
                        {customers.map((customer) => {
                          const isSuggested =
                            customer.id === suggestedCustomerId;
